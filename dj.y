...
int main(int argc, char **argv) {
    ...
    if (argc != 2) {
        fprintf(stderr, "Usage: djc filename.dj\n");
        exit(-1);
    }

    /* Ensure the input file has a .dj extension */
    const char *inName = argv[1];
    size_t len = strlen(inName);
    if (len < 4 || strcmp(inName + len - 3, ".dj") != 0) {
        fprintf(stderr, "Error: Input file must have .dj extension\n");
        fprintf(stderr, "Usage: djc filename.dj\n");
        exit(-1);
    }
    ...
    /* Build the output filename by copying and replacing the extension */
    char outputFile[256];
    strcpy(outputFile, inName);
    char *outExt = outputFile + (len - 3);  /* points to the ".dj" */
    strcpy(outExt, ".dism");                /* now outputFile ends in ".dism" */
